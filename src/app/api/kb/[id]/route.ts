...
export const PATCH = withOrgRoles(["owner", "admin"], async (session, req: Request, ctx: Params) => {
...
export const DELETE = withOrgRoles(["owner", "admin"], async (session, _req: Request, ctx: Params) => {
  const { id } = await ctx.params;
  const db = getDb();
